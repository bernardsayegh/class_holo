...
### 4.6 ΛCDM + Mapping Controls (acdm)

Tests the geometric mapping *without* holographic braking. Demonstrates that mapping alone cannot resolve the tensions — β is doing the real work on S₈.

```ini
# acdm with Amap = 2
interaction_beta = 0              # No holographic interaction
super_schwarzschild_correction = yes
super_schw_amp = 0.0
super_schw_Amap = 2.0
super_schw_deltaS = 0.03
...
Without β, there is no S₈ suppression and no X_schw accumulation (S never exceeds 1 in ΛCDM), so the mapping has nothing to act on. H₀_local = H₀_phys identically.

Two MCMC variants:
- `acdm_Amap_free.input.yaml` — Amap free in MCMC
- `acdm_mapping_Amap2.input.yaml` — Amap fixed at 2

These produce Table 4 in the paper. The local-frame quantities are also diagnostic:

```
Ω_m,loc = ω_m / (H₀_local/100)²
S₈,loc  = σ₈ √(Ω_m,loc / 0.3)
```

For acdm, S₈,loc = S₈ since H₀_local = H₀. For Model B, S₈,loc < S₈ because the larger H₀_local pushes Ω_m,loc below Ω_m.

Validation script: `scripts/check_acdm_mapping.py` runs all four cases (ΛCDM, acdm, Model A, Model B) at fixed h and prints the comparison table.

### 4.7 Far-Future Completion (Optional)
...
- **acdm**: Identical to ΛCDM — mapping infrastructure present but inert without β (S < 1, X0 = 0)
...
- **S8_loc < S8** for Models B and C because Om_loc < Om (same ωm, larger H0_local)
...
| Model | H₀_phys | H₀_local | σ₈ | S₈ | S₈,loc |
|-------|---------|----------|-----|-----|--------|
| ΛCDM | 67.2 | 67.2 | 0.808 | 0.828 | 0.828 |
| acdm (Amap=2) | 67.2 | 67.2 | 0.808 | 0.828 | 0.828 |
| Model A | 67.3 | 67.3 | 0.743 | 0.778 | 0.778 |
| Model B | 67.3 | 72.2 | 0.743 | 0.778 | 0.761 |
| Model C | 68.3 | 73.2 | 0.737 | 0.762 | 0.744 |
| Model D | 68.3 | 68.3 | 0.737 | 0.762 | 0.762 |
